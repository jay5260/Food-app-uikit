...
class RestaurantMainPageVC: UIViewController, StoryboardInstantiable {
    static var storyboardName: String = StoryboardNames.restaurantPage.name
    ...
    //MARK: - Variables
    var viewModel = RestaurantMainPageViewModel()
...
//MARK: - Intial setup
extension RestaurantMainPageVC {
    private func initialSetup() {
        ...
        tableView.register(UINib(nibName: "TestimonialTblCell", bundle: nil), forCellReuseIdentifier: "TestimonialTblCell")
    }
}


//MARK: - Collectionview delegate & datasorouce
extension RestaurantMainPageVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension RestaurantMainPageVC: UITableViewDelegate, UITableViewDataSource {
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.testimonialArr.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
//        let cell = tableView.dequeueReusableCell(withIdentifier: "MenuTblCell", for: indexPath) as! MenuTblCell
        let cell = tableView.dequeueReusableCell(withIdentifier: "TestimonialTblCell", for: indexPath) as! TestimonialTblCell
        cell.layer.cornerRadius = 8 // Adjust the corner radius as needed
        cell.layer.masksToBounds = false
        cell.layer.shadowColor = UIColor.black.cgColor
        cell.layer.shadowOpacity = 0.1  
        cell.layer.shadowOffset = CGSize(width: 0, height: 6)
        cell.layer.shadowRadius = 5
        cell.config(data: viewModel.testimonialArr[indexPath.row])
        return cell
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc = PrivateProfileVC.instantiate()
